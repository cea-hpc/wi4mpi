...
typedef enum A_MPIR_Win_flavor {
...
/* Topology types */
typedef enum A_MPIR_Topo_type { A_MPI_GRAPH=1, A_MPI_CART=2, A_MPI_DIST_GRAPH=3 } A_MPIR_Topo_type;
...
/* MPIR_ERRORS_THROW_EXCEPTIONS is not part of the MPI standard, it is here to
   ...
   the standard. */
#define A_MPIR_ERRORS_THROW_EXCEPTIONS ((A_MPI_Errhandler)0x54000002)
...
/* I_A_MPI_VERSION is the version string. I_A_MPI_NUMVERSION is the
 * numeric version that can be used in numeric comparisons.
 *
 * I_A_MPI_VERSION uses the following format:
 * Version: [MAJ].[MIN].[REV][EXT][EXT_NUMBER]
 * Example: 1.0.7rc1 has
 *          MAJ = 1
 *          MIN = 0
 ...
 *          EXT_NUMBER = 1 
 *
 * I_A_MPI_NUMVERSION will convert EXT to a format number:
 *          ALPHA (a) = 0
 *          BETA (b)  = 1
 *          RC (rc)   = 2
 *          PATCH (p) = 3
 ...
 * 1.0.7rc1 will have the numeric version 10007201.
 */
#define I_A_MPI_VERSION "5.1.1"
#define I_A_MPI_NUMVERSION 50101300

/* for the datatype decoders */
enum A_MPIR_Combiner_enum {
...
} A_MPIR_T_verbosity_t;

typedef enum A_MPIR_T_bind_t {
    /* don't name-shift this if/when A_MPI_T_ is accepted, this is an MPICH-only
     * extension */
    A_MPIX_T_BIND_INVALID = 0,
...
typedef enum A_MPIR_T_scope_t {
    /* don't name-shift this if/when A_MPI_T_ is accepted, this is an MPICH-only
     * extension */
    A_MPIX_T_SCOPE_INVALID = 0,
...
typedef enum A_MPIR_T_pvar_class_t {
    /* don't name-shift this if/when A_MPI_T_ is accepted, this is an MPICH-only
     * extension */
    A_MPIX_T_PVAR_CLASS_INVALID = 0,

    /* arbitrarily shift values to aid debugging and reduce accidental errors */
    A_MPIR_T_PVAR_CLASS_FIRST = 240,
    A_MPI_T_PVAR_CLASS_STATE = A_MPIR_T_PVAR_CLASS_FIRST,
    ...
    A_MPIR_T_PVAR_CLASS_LAST,
    A_MPIR_T_PVAR_CLASS_NUMBER = A_MPIR_T_PVAR_CLASS_LAST - A_MPIR_T_PVAR_CLASS_FIRST
} A_MPIR_T_pvar_class_t;
